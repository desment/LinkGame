...
(function (window, undefined) {
	...
	var Doudou = function (name) {
		...
		this.SoundBGFile = (Math.round(Math.random()) == 0) ? 'sound.GameSceneBGM.mp3' : 'sound.WorldSceneBGM.mp3';
		this.SoundBG = new Audio(this.SoundBGFile);
		this.SoundCG = new Audio("sound.level.1.mp3");
		this.SoundSelect = new Audio('sound.select.mp3');
		this.SoundDisappear = new Audio('sound.dispear.1.mp3');
		this.SoundResort = new Audio('sound.resort.mp3');
		this.SoundNoDisappear = new Audio('sound.nodispear.mp3');
	...
	Doudou.prototype = {
		...
		drawBox : function (ctx, y, x, color) {
			...
			ctx.closePath();
			if (color) {
				ctx.drawImage(this.img, 1, this.imgPosition[color], this.imgWidthArray[color], this.imgHeightArray[color],
					x * this.picWidth + this.leftPadding, y * this.picHeight + this.topPadding, this.picWidth, this.picHeight);
			}
		},
